...
import { getStock, updateStock, deleteStock } from "@/modules/stocks";
...
export async function GET(_req: Request, { params }: Ctx) {
  ...
  const r = await getStock(params.id);
  if (!r.ok) return mapError(r.error);
  return NextResponse.json({ stock: r.value });
}
...
export async function PATCH(req: Request, { params }: Ctx) {
  ...
  const r = await updateStock(params.id, raw);
  if (!r.ok) return mapError(r.error);
  return NextResponse.json({ stock: r.value });
}
...
export async function DELETE(_req: Request, { params }: Ctx) {
  if (!isUuid(params.id)) return badId(params.id);
  const r = await deleteStock(params.id);
  if (!r.ok) return mapError(r.error);
  return NextResponse.json({ stock: r.value });
}
...
function mapError(err: { kind: string; message?: string; errors?: unknown }) {
  switch (err.kind) {
    ...
    case "duplicate_sku":
      return NextResponse.json({ error: "duplicate_sku", message: err.message }, { status: 409 });
    default:
